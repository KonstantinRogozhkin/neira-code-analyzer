...
## ⚖️ Разделение ответственности: FilterAnalyzer и AnalysisReporter

В рамках рефакторинга для соблюдения **Принципа единственной ответственности (SRP)**, монолитный сервис `ContextGenerator` был разделен на два специализированных класса:

1.  **`FilterAnalyzer`** (`src/neira_code_analyzer/filter_analyzer.py`)
    *   **Ответственность:** Только анализ файловой структуры, подсчет токенов и применение фильтров.
    *   Не занимается созданием отчетов или форматированием вывода.
    *   **Вход:** Путь к проекту, паттерны включения/исключения.
    *   **Выход:** Структурированные данные с результатами анализа (статистика по файлам, размеры).

2.  **`AnalysisReporter`** (`src/neira_code_analyzer/analysis_reporter.py`)
    *   **Ответственность:** Только генерация человекочитаемых и машиночитаемых отчетов на основе готовых данных.
    *   **Вход:** Результаты работы `FilterAnalyzer`.
    *   **Выход:** Отформатированный Markdown-отчет.

**Преимущества этого разделения:**
-   **Модульность:** Каждый класс решает одну задачу.
-   **Тестируемость:** Легко писать юнит-тесты для каждого компонента в изоляции.
-   **Гибкость:** Можно легко добавить новые форматы отчетов (например, JSON или HTML), не затрагивая логику анализа.
...
- **Рефакторинг `docs_generator.py`** - переход на JSON-контракт с ИИ для большей надежности и разделение на более мелкие сервисы (`DocsSessionManager`, `ActionExecutor`, `DocsAgent`).
- **Динамическая генерация схем MCP** - устранение дублирования названий пресетов в `mcp_schemas.py` за счет их генерации из доступных ключей.
